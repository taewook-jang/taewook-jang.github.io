...
Kotlin에서는 `constructor`을 여러 개로 정의할 수 있는 대 다음과 같은 형태로 생성자 정의가 가능합니다.
...
추가로 kotlin의 `open class`를 추가하여 다음과 같이 확장도 가능합니다.
